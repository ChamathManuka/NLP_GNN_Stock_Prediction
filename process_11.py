...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        ...
        merged_df.to_pickle("process_11_files/list_files/"+stock_name+"_merged_data_with_daily_return.pkl")
...
def model_building(stock_name):
    df = pd.read_pickle("process_11_files/list_files/"+stock_name+"_merged_data_with_daily_return.pkl")
    ...
    X = df.drop(columns=['added_date', 'Daily Return', 'Close (Rs.)', 'blob_sentiment', 'compound', 'neutral_sentiment','RSI_quantile', 'MACD_quantile', 'ATR_quantile', 'SMA_10_quantile', 'EMA_10_quantile', 'OBV_quantile',
    ...
    model = Sequential([
        Dense(128, activation='relu', input_shape=(102,)),  # Input layer
