...
from keras.src.layers import BatchNormalization, Dropout, LeakyReLU
...
def data_preprocessing():
    ...
    def preprocessing():
        ...
        df_grouped['vector'] = [get_sentence_embedding(doc) for doc in processed_documents]

        # Save the model and DataFrame
        df_grouped.to_pickle("process_13_files/list_files/bert_vector.pkl")
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        df1 = pd.read_pickle("process_13_files/list_files/bert_vector.pkl")
        ...
        merged_df.to_pickle("process_13_files/list_files/"+stock_name+"_merged_data_with_daily_return.pkl")

        print("Merged data saved process_13_files directory")
...
def model_building(stock_name):
    df = pd.read_pickle("process_13_files/list_files/"+stock_name+"_merged_data_with_daily_return.pkl")
    ...
    X = df.drop(columns=['added_date', 'Daily Return', 'Close (Rs.)', 'blob_sentiment', 'compound','negative_sentiment','positive_sentiment', 'neutral_sentiment','RSI_quantile', 'MACD_quantile', 'ATR_quantile', 'SMA_10_quantile', 'EMA_10_quantile', 'OBV_quantile',
    ...
    X_train_extra = X_train_scaled[:, 768:]
    X_test_extra = X_test_scaled[:, 768:]

    pca = PCA(n_components=300)  # Reduce from 768D to 300D
    X_train_scaled = np.hstack((pca.fit_transform(X_train_scaled[:,:768]), X_train_extra))
    X_test_scaled = np.hstack((pca.transform(X_test_scaled[:,:768]), X_test_extra))
    print(f"Explained Variance: {sum(pca.explained_variance_ratio_)}")

    def build_model(hp):
        model = Sequential([
            Dense(hp.Int('units_1', min_value=0, max_value=1024, step=64),
                  kernel_regularizer=l2(0.01), input_shape=(301,)),
            LeakyReLU(alpha=0.1),
            BatchNormalization(),
            Dropout(0.4),
    ...
    #                         directory='my_dir', project_name='model_tuning_'+stock_name+'_3')
    ...
    model = Sequential([ #HNB
        Dense(896, activation='relu', kernel_regularizer=l2(0.01), input_shape=(301,)),
        BatchNormalization(),
        LeakyReLU(alpha=0.1),
        Dropout(0.3),
    ...
    #     Dense(896, activation='relu', kernel_regularizer=l2(0.01), input_shape=(300,)),
    #     BatchNormalization(),
    #     LeakyReLU(alpha=0.1),
    #     Dropout(0.3),
    #
