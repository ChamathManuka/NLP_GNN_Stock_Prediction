...
sentiment_sim = cosine_similarity(df[['neu_sent', 'pos_sent', 'neg_sent']].values)

# Add news-news edges based on similarity
for i in range(len(df)):
    for j in range(i + 1, len(df)):
        sim_weight = (sbert_sim[i, j] + fingpt_sim[i, j] + sentiment_sim[i, j]) / 3  # Average similarity
        if sim_weight > 0.6:
...
# Add stock-stock edges (time-based)
unique_dates = sorted(df['added_date'].unique())
for i in range(len(unique_dates) - 1):
    graph.add_edge(f"stock_{unique_dates[i]}", f"stock_{unique_dates[i + 1]}", weight=np.tanh(df[df['added_date'] == unique_dates[i]]['return_scaled'].values[0]))
