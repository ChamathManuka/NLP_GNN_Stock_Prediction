...
return_scaler = joblib.load("../vectorization_process_files/model_files/return_scaler.pkl")
price_scaler = joblib.load("../vectorization_process_files/model_files/min_max_scaler.pkl")
...
for index, row in df_test.iterrows():
    news_vector = np.concatenate([row['doc2vec_vectors'], row['tfidf_vectors']])
...
sbert_sim_test = cosine_similarity(np.stack(df_test['doc2vec_vectors'].tolist()))
fingpt_sim_test = cosine_similarity(np.stack(df_test['tfidf_vectors'].tolist()))

# 7️⃣ Add news-news edges based on similarity
for i in range(len(df_test)):
    for j in range(i + 1, len(df_test)):
        sim_weight = (sbert_sim_test[i, j] + fingpt_sim_test[i, j]) / 2  # Average similarity
        if sim_weight > 0.6:
...
loaded_model = load_model("../vectorization_process_files/model_files/graphsage_stock_model.h5", custom_objects=custom_objects)
