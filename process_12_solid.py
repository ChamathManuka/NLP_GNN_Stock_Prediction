...
def data_preprocessing():
    ...
    def tfidf_calculation(documents, dataframe):
        ...
        tfidf_vectorizer = TfidfVectorizer(
            ...
            max_df=0.5,  # Ignore terms that appear in more than 70% of documents
            ...
            max_features=700  # Limit to 5000 most important features
        )
...
def model_building(stock_name):
    ...
    X_train_scaled = scaler.fit_transform(X_train[:, 2:702])
    X_test_scaled = scaler.transform(X_test[:,2 :702])

    # pca = PCA(n_components=150)  #
    # X_train_scaled = pca.fit_transform(X_train_scaled)
    # X_test_scaled = pca.transform(X_test_scaled)

    selector = RFE(Ridge(alpha=1.0), n_features_to_select=600)  # Keep 150 best features
    ...
    model = Sequential([ #HNB
        Dense(896, activation='relu', kernel_regularizer=l2(0.01), input_shape=(602,)),
        BatchNormalization(),
        LeakyReLU(alpha=0.1),
        Dropout(0.2),
