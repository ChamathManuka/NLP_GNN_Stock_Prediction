...
def data_cleaning():
    ...
    df = pd.read_csv("All_news_CSV_files/old_news_articles.csv", encoding="ISO-8859-1")  # Replace with the actual encoding
    df.to_csv("All_news_CSV_files/old_news_articles.csv", index=False, encoding='utf-8')
    ...
    processed_df.to_csv("process_7.2_files/csv_files/old_news_articles_processed_not_concat.csv", index=False)
#--------------------------------------------------------------------------------------------------------
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        ...
        df1 = pd.read_csv("process_7.2_files/csv_files/old_news_articles_processed_not_concat.csv")
        ...
        merged_df.to_csv('process_7.2_files/csv_files/' + stock_name + '_merged_data_with_daily_return.csv',
                         index=False)
        print("Merged data saved process_7.2_files directory")
...
#--------------------------------------------------------------------------------------------------------
def observe_correlations(stock_name):
    ...
    df = pd.read_csv('process_7.2_files/csv_files/' + stock_name + '_merged_data_with_daily_return.csv')
    processed_data = []
    for index, row in df.iterrows():
        ...
        if (isinstance(article, str)):
            ...
            processed_data.append({"added_date": row["added_date"], "filtered_tokens": article, "daily_return": row['Daily_Return'],
...
#--------------------------------------------------------------------------------------------------------
def visual_correlations(stock_name):
...
def sentiment_vectorization(stock_name):
    ...
    def create_model_data(stock_name):
        # Load data from CSV
        data = pd.read_csv( 'process_7.2_files/csv_files/' + stock_name + '_merged_data_with_daily_return.csv')
    ...
    with open('process_7.2_files/list_files/'+stock_name+'_embeddings_bert.pkl', 'wb') as f:
        pickle.dump(new_embeddings, f)
    with open('process_7.2_files/list_files/'+stock_name+'_significant_dimensions.pkl', 'wb') as f:
        pickle.dump(significant_dimensions, f)
    with open('process_7.2_files/list_files/'+stock_name+'_daily_returns.pkl', 'wb') as f:
...
def create_model(stock_name):
    ...
    with open('process_7.2_files/list_files/' + stock_name + '_embeddings_bert.pkl', 'rb') as f:
        embeddings = pickle.load(f)
    with open('process_7.2_files/list_files/' + stock_name + '_significant_dimensions.pkl', 'rb') as f:
        significant_dimensions = pickle.load(f)
    with open('process_7.2_files/list_files/' + stock_name + '_daily_returns.pkl', 'rb') as f:
        daily_returns = pickle.load(f)
    with open('process_7.2_files/list_files/' + stock_name + '_test_june_to_sept_vectorized.pkl', 'rb') as f:
    ...
    # torch.save(model.state_dict(), 'process_7.2_files/model_files/' + stock_name + '_lstm_model.pth')

    # loaded_model = LSTMModel(len(significant_dimensions), hidden_size, num_layers)
    # loaded_model.load_state_dict(torch.load('process_7.2_files/model_files/' + stock_name + '_lstm_model.pth'))
    # loaded_model.eval()
...
# observe_correlations("JKH")
visual_correlations("JKH")
