...
def data_cleaning():
    ...
    processed_df.to_csv("process_4_files/csv_files/old_news_articles_processed.csv", index=False)
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        df1 = pd.read_csv("../process_4_files/csv_files/old_news_articles_processed.csv")
...
def sentiment_vectorization(stock_name):
    ...
    def find_significant_features(embeddings, daily_returns):
        ...
        significant_dimensions = np.where(p_values < 0.03)[0]
    ...
    with open('process_4_files/list_files/' + stock_name + '_embeddings_bert.pkl', 'wb') as f:
        pickle.dump(new_embeddings, f)
    with open('process_4_files/list_files/' + stock_name + '_significant_dimensions.pkl', 'wb') as f:
        pickle.dump(significant_dimensions, f)
    with open('process_4_files/list_files/' + stock_name + '_daily_returns.pkl', 'wb') as f:
...
def create_model(stock_name):
    with open('process_4_files/list_files/' + stock_name + '_embeddings_bert.pkl', 'rb') as f:
    ...
    with open('process_4_files/list_files/' + stock_name + '_daily_returns.pkl', 'rb') as f:
    ...
    torch.save(model.state_dict(), 'process_4_files/model_files/' + stock_name + '_lstm_model.pth')
