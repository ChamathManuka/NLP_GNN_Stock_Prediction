...
from keras.src.optimizers import RMSprop, SGD
...
def data_preprocessing():
    ...
    def tfidf_calculation(documents, dataframe):
        ...
        tfidf_vectorizer = TfidfVectorizer(
            ...
            max_df=0.6,  # Ignore terms that appear in more than 70% of documents
            ...
            max_features=250  # Limit to 5000 most important features
        )
        tfidf_matrix = tfidf_vectorizer.fit_transform(documents)
        joblib.dump(tfidf_vectorizer, "../process_14_files/model_files/tfidf_vectorizer.pkl")
        joblib.dump(tfidf_matrix, "../process_14_files/model_files/tfidf_matrix.pkl")
    ...
    def preprocessing():
        ...
        # Train Doc2Vec model
        model = Doc2Vec(vector_size=300, min_count=2, epochs=60, workers=6)
        ...
        model.save("process_14_files/model_files/doc2vec_model.model")
        df_grouped.to_pickle("process_14_files/list_files/document_vectors.pkl")
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        df1 = pd.read_pickle("../process_14_files/list_files/document_vectors.pkl")
        ...
        merged_df.to_pickle("process_14_files/list_files/" + stock_name + "_merged_data_with_daily_return.pkl")

        print("Merged data saved process_14_files directory")
...
def model_building_1(stock_name):
    df = pd.read_pickle("process_14_files/list_files/" + stock_name + "_merged_data_with_daily_return.pkl")
    ...
    model.save("process_14_files/model_files/" + stock_name + "_sequential.h5")
    # Make predictions
...
def model_building(stock_name):
    ...
    X = df["previous_return"].to_numpy(dtype=np.float32)
    ...
    # scaler = StandardScaler()
    # X_train_extra = X_train[:, 250:]
    # X_test_extra = X_test[:, 250:]
    #
    # X_train_scaled = scaler.fit_transform(X_train[:, :250])
    # X_test_scaled = scaler.transform(X_test[:, :250])

    # pca = PCA(n_components=150)  #
    # X_train_scaled = pca.fit_transform(X_train_scaled)
    # X_test_scaled = pca.transform(X_test_scaled)

    # selector = RFE(Ridge(alpha=1.0), n_features_to_select=150)  # Keep 150 best features
    ...
    def build_model(hp):
        model = Sequential([
            Dense(hp.Int('units_1', min_value=0, max_value=1024, step=64),
                  kernel_regularizer=l2(0.01), input_shape=(300,)),
            LeakyReLU(alpha=0.1),
            BatchNormalization(),
            Dropout(0.4),

            Dense(hp.Int('units_2', min_value=0, max_value=512, step=64),
                  kernel_regularizer=l2(0.01)),
            LeakyReLU(alpha=0.1),
            BatchNormalization(),
            Dropout(0.4),

            Dense(hp.Int('units_3', min_value=0, max_value=256, step=64),
                  kernel_regularizer=l2(0.01)),
            LeakyReLU(alpha=0.1),
            BatchNormalization(),
    ...
    #                         directory='E:\Academic\master\master\parameter_tuning', project_name='model_tuning_'+stock_name+'_7')
    ...
    model = Sequential([  # HNB
        Dense(896, activation='relu', kernel_regularizer=l2(0.01), input_shape=(1,)),
        BatchNormalization(),
        LeakyReLU(alpha=0.1),
        Dropout(0.2),
    ...
    #     Dense(896, activation='relu', kernel_regularizer=l2(0.01), input_shape=(152,)),
    #     BatchNormalization(),
    #     LeakyReLU(alpha=0.1),
    #     Dropout(0.3),
    #
    ...
    print(f"Mean Absolute Error (HUBER): {huber}")
    print(f"R² Score: {r2}")
