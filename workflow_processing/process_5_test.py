...
def data_cleaning():
    ...
    processed_data = []
    df = pd.read_csv("../All_news_CSV_files/business_test_articles.csv")
    ...
    processed_df.to_csv("process_5_files/csv_files/business_test_articles_processed.csv", index=False)
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        df1 = pd.read_csv("../process_5_files/csv_files/business_test_articles_processed.csv")
...
def sentiment_vectorization(stock_name):
    ...
    def create_model_data(stock_name):
        ...
        with open('process_5_files/list_files/' + stock_name + '_test_business_vectorized.pkl', 'wb') as f:
...
def make_prediction(stock_name):
    with open('process_5_files/list_files/' + stock_name + '_test_business_vectorized.pkl', 'rb') as f:
    ...
    loaded_model.load_state_dict(torch.load('process_5_files/model_files/' + stock_name + '_lstm_model.pth'))
    loaded_model.eval()
    ...
    df_new.to_csv('process_5_files/prediction_files/' + stock_name + '_test_business_predicted.csv')


# --------------------------------------------------------------------------------------------------------
def prediction_graph(stock_name):
    # Load the CSV file
    df = pd.read_csv('process_5_files/prediction_files/' + stock_name + '_test_business_predicted.csv')
...
def business_date_selection_graph(stock_name):
    ...
    df.to_csv('process_5_files/csv_files/' + stock_name + '_test_business_predicted_business_dates.csv', index=False)
...
def correlation_graph(stock_name):
    # Load the CSV file
    df = pd.read_csv('process_2_files/' + stock_name + '_test_business_predicted_business_dates.csv')
    # Convert 'added_date' to datetime and extract date only
    df['added_date'] = pd.to_datetime(df['added_date']).dt.date
