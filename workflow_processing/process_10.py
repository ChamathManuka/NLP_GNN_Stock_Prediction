...
def calculate_sentiment(stock_name):
    ...
    def write_sentiment_to_file():
            ...
            if (isinstance(article, str)):
                ...
                processed_data.append(
                    {"added_date": row["added_date"], "filtered_tokens": article, "daily_return": row["Daily_Return"],
                     "positive sentiment": sentiment.get("pos"), "negative sentiment": sentiment.get("neg"),
        ...
        processed_df.to_csv(
            'process_10_files/csv_files/' + stock_name + '_merged_data_with_daily_return_and_sentiment.csv')
    ...
    def correlation_analysis(stock_name):
        ...
        grouped_df.to_csv(
            'process_10_files/csv_files/' + stock_name + '_merged_data_with_daily_return_and_sentiment_grouped.csv',
            index=False)
...
def correlation_analysis(stock_name):
    ...
    df[['daily_return', 'positive sentiment', 'neutral sentiment', 'negative sentiment']] = scaler.fit_transform(
        df[['daily_return', 'positive sentiment', 'neutral sentiment', 'negative sentiment']])
