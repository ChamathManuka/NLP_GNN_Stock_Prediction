...
def data_preprocessing():
    ...
    def tfidf_calculation(documents, dataframe):
        tfidf_vector = []
        tfidf_vectorizer = joblib.load("../process_12_files/model_files/tfidf_vectorizer.pkl")
    ...
    def preprocessing():
        ...
        # model = Doc2Vec.load("process_11_files/model_files/doc2vec_model.model")
        ...
        df_grouped.to_pickle("process_11_files/list_files/document_vectors_test.pkl")
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        df1 = pd.read_pickle("../process_11_files/list_files/document_vectors_test.pkl")
        ...
        merged_df.to_pickle("process_11_files/list_files/" + stock_name + "_merged_data_with_daily_return_test.pkl")
...
def prediction(stock_name):
    df = pd.read_pickle("process_11_files/list_files/" + stock_name + "_merged_data_with_daily_return_test.pkl")
    ...
    # df['vector_scaled'] = scaler.fit_transform(df['vector'])
    # X = df.drop(columns=['added_date', 'Daily Return', 'Close (Rs.)', 'blob_sentiment', 'compound'])  # Features
    # X = df.drop(columns=['added_date', 'Daily Return', 'Close (Rs.)', 'blob_sentiment', 'compound', 'vector',
    #                      'positive_sentiment', 'negative_sentiment',
    #                      'neutral_sentiment'])  # Features ,'positive_sentiment', 'negative_sentiment','neutral_sentiment','RSI_quantile', 'MACD_quantile', 'ATR_quantile', 'SMA_10_quantile', 'EMA_10_quantile', 'OBV_quantile', 'CMF_quantile'
    ...
    X_test_extra = X[:, 0:2]
    X_test_scaled = scaler.transform(X[:, 2:502])

    selector = joblib.load("process_15_files/model_files/" + stock_name + "_selector.pkl")
    X_test_selected = selector.transform(X_test_scaled)

    X_test_scaled = np.hstack((X_test_selected, X_test_extra))

    loaded_model = load_model("process_12_files/model_files/" + stock_name + "_sequential.h5")
