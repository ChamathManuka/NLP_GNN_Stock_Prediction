...
from keras.src.layers import LeakyReLU, BatchNormalization, Dropout
...
def data_preprocessing(stock_name):
    ...
    def tfidf_calculation(documents, dataframe):
        ...
        joblib.dump(tfidf_vectorizer, "process_12.1_files/model_files/tfidf_vectorizer.pkl")
        joblib.dump(tfidf_matrix, "process_12.1_files/model_files/tfidf_matrix.pkl")
    ...
    def preprocessing():
        ...
        df1 = pd.read_csv("stock_price_files/" + stock_name + ".csv")
        df1['Trade Date'] = pd.to_datetime(df1['Trade Date']).dt.date

        merged_df = pd.merge(df1, df_grouped, left_on='Trade Date', right_on='added_date', how='inner')
        merged_df = merged_df.drop(
            columns=['added_date', 'Open (Rs.)', 'High (Rs.)', 'Low (Rs.)', 'ShareVolume', 'Turnover (Rs.)'], axis=1)

        merged_df['Trade Date'] = pd.to_datetime(merged_df['Trade Date'])
        full_date_range = pd.DataFrame(
            {'Trade Date': pd.date_range(merged_df['Trade Date'].min(), merged_df['Trade Date'].max())})
        df_merged = full_date_range.merge(merged_df, on='Trade Date', how='left')
        # Iterate through rows and handle missing data
        filtered_tokens_accumulated = []  # Stores accumulated tokens
        new_rows = []  # Stores processed rows

        for i, row in df_merged.iterrows():
            if pd.isna(row['filtered_tokens']):  # If row is missing
                continue  # Skip missing rows

            # If there were missing days before, concatenate previous tokens
            if filtered_tokens_accumulated:
                row['filtered_tokens'] = sum(filtered_tokens_accumulated, []) + row['filtered_tokens']
                filtered_tokens_accumulated = []  # Reset accumulator

            new_rows.append(row)  # Save row

            # Store the current day's tokens for possible missing days
            filtered_tokens_accumulated.append(row['filtered_tokens'])

        # Convert back to DataFrame
        df_fixed = pd.DataFrame(new_rows)

        # Fill NaN values in sentiment columns with rolling mean
        sentiment_columns = ['positive_sentiment', 'negative_sentiment', 'neutral_sentiment', 'compound',
                             'blob_sentiment']
        df_fixed[sentiment_columns] = df_fixed[sentiment_columns].fillna(method='ffill')

        # Reset index
        df_fixed.reset_index(drop=True, inplace=True)

        tf_idf_list = [' '.join(row) for row in df_fixed['filtered_tokens']]
        tfidf_calculation(tf_idf_list, df_fixed)

        df_fixed.to_pickle("process_12.1_files/list_files/document_vectors.pkl")
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        df1 = pd.read_pickle("process_12.1_files/list_files/document_vectors.pkl")
        ...
        merged_df.to_pickle("process_12.1_files/list_files/" + stock_name + "_merged_data_with_daily_return.pkl")

        print("Merged data saved process_12.1_files directory")
...
def model_building_1(stock_name):
    df = pd.read_pickle("process_12.1_files/list_files/" + stock_name + "_merged_data_with_daily_return.pkl")
    ...
    model.save("process_12.1_files/model_files/" + stock_name + "_sequential.h5")
    # Make predictions
...
def model_building(stock_name):
    ...
    X = df.drop(columns=['positive_sentiment', 'negative_sentiment', 'neutral_sentiment', 'added_date', 'Daily Return',
                         'Close (Rs.)', 'RSI_quantile', 'MACD_quantile', 'ATR_quantile', 'SMA_10_quantile',
                         'EMA_10_quantile', 'OBV_quantile',
    ...
    X_train_extra = X_train[:, 0:3]
    X_test_extra = X_test[:, 0:3]

    X_train_scaled = scaler.fit_transform(X_train[:, 3:503])
    X_test_scaled = scaler.transform(X_test[:, 3:503])
    ...
    model = Sequential([  # HNB
        Dense(896, activation='relu', kernel_regularizer=l2(0.01), input_shape=(493,)),
        BatchNormalization(),
        LeakyReLU(alpha=0.1),
        Dropout(0.2),
