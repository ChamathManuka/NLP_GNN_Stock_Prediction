...
def data_cleaning():
    ...
    df_grouped.to_csv("process_7_files/csv_files/test_june_to_sept_processed.csv", index=False)
...
def stock_data_preprocessing(stock_name):
    ...
    def merge_daily_returns():
        ...
        df1 = pd.read_csv("../process_7_files/csv_files/test_june_to_sept_processed.csv")
        ...
        merged_df.to_csv('process_7_files/csv_files/' + stock_name + '_merged_data_with_daily_return.csv',
                         index=False)
        print("Merged data saved process_7_files directory")
...
def sentiment_vectorization(stock_name):
    ...
    def create_model_data(stock_name):
        ...
        def find_significant_features(embedding):
            with open('process_7_files/list_files/' + stock_name + '_significant_dimensions.pkl', 'rb') as f:
                significant_dimensions = pickle.load(f)
            return embedding[:, significant_dimensions]

        # Load data from CSV
        data = pd.read_csv( 'process_7_files/csv_files/' + stock_name + '_merged_data_with_daily_return.csv')
        ...
        with open('process_7_files/list_files/' + stock_name + '_test_june_to_sept_vectorized.pkl', 'wb') as f:
...
def make_prediction(stock_name):
    with open('process_7_files/list_files/' + stock_name + '_test_june_to_sept_vectorized.pkl', 'rb') as f:
    ...
    loaded_model.load_state_dict(torch.load('process_7_files/model_files/'+stock_name+'_lstm_model.pth'))
    loaded_model.eval()
    ...
    df_new.to_csv('process_7_files/prediction_files/' + stock_name + '_test_june_to_sept_predicted.csv')
#--------------------------------------------------------------------------------------------------------
def prediction_graph(stock_name):

    # Load the CSV file
    df = pd.read_csv('process_7_files/prediction_files/' + stock_name + '_test_june_to_sept_predicted.csv')
...
def business_date_selection_graph(stock_name):
    ...
    df.to_csv('process_7_files/csv_files/'+stock_name+'_test_june_to_sept_predicted_business_dates.csv', index=False)
