...
# 1️⃣ Load Data
df = pd.read_pickle("vectorization_process_files/list_files/train.pkl")
...
# Save return scaler
joblib.dump(return_scaler, "vectorization_process_files/model_files/return_scaler.pkl")
...
for index, row in df.iterrows():
    news_vector = np.concatenate([row['doc2vec_vectors'], row['tfidf_vectors']])  # Removed sentiment values
...
sbert_sim = cosine_similarity(np.stack(df['doc2vec_vectors'].tolist()))
fingpt_sim = cosine_similarity(np.stack(df['tfidf_vectors'].tolist()))

# 6️⃣ Add news-news edges based on similarity
for i in range(len(df)):
    for j in range(i + 1, len(df)):
        sim_weight = (sbert_sim[i, j] + fingpt_sim[i, j]) / 2  # Average similarity
...
joblib.dump(scaler, "vectorization_process_files/model_files/min_max_scaler.pkl")
...
# 1️⃣5️⃣ Save Model
model.save("vectorization_process_files/model_files/graphsage_stock_model.h5")
...
loaded_model = load_model(
    "vectorization_process_files/model_files/graphsage_stock_model.h5", custom_objects=custom_objects
)
